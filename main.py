...
@app.post("/pocket-option/manual-login/start/{telegram_user_id}")
async def pocket_option_manual_login_start(telegram_user_id: str):
    try:
        return await pocket_option_client.start_manual_login(telegram_user_id)
    except Exception as e:
        raise HTTPException(502, f"Failed to start manual login: {e}")


@app.post("/pocket-option/manual-login/complete/{telegram_user_id}")
async def pocket_option_manual_login_complete(telegram_user_id: str, mode: str = "demo"):
    if mode not in ("demo", "live"):
        raise HTTPException(400, "mode must be 'demo' or 'live'")
    try:
        return await pocket_option_client.complete_manual_login(telegram_user_id, mode)
    except Exception as e:
        raise HTTPException(502, f"Failed to complete manual login: {e}")
...
async def pocket_option_connect(telegram_user_id: str, mode: str = "demo"):
    if mode not in ("demo", "live"):
        raise HTTPException(400, "mode must be 'demo' or 'live'")
    try:
        result = await pocket_option_client.login(telegram_user_id, mode)
        return result
